...
export const createConfigStorage = (opts: Pick<ConfigLoaderOptions, 'defaultConfigPath'|'userConfigPath'|'envMode'>): Record<any, any> => {
    ...
    let envConfig = {};
    if (opts.envMode === 'default') {
        envConfig = opts.envMode ? fromPairs(keys(defaultConfig).map(k => [k, process.env[k]])) : {};
    } else if (opts.envMode === 'merged') {
        envConfig = opts.envMode ? fromPairs([...keys(defaultConfig), ...keys(userConfig)].map(k => [k, process.env[k]])) : {};
